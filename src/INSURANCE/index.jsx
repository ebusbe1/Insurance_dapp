    ...
    Promise.all([
      ctcInsurance.p.Insurance_company({
        ...
        Check_pay: async (sub_pay) => {
          console.log(
            `seen cash :  ${stdlib.formatCurrency(sub_pay)} ${
              stdlib.standardUnit
            }`
          );

          if (sub_pay >= Number(stdlib.parseCurrency(1))) {
    ...
    <div className="container">
      ...
      <div className="Navbar">
        <h1>
          ...
          <ConnectWalletButton />
          <div onClick={deployContract} className="connect">
            Deploy
          </div>
        </h1>
...
const Card = ({
  ...
  const handleClaimPlan = async (claim, fund) => {
    try {
      ...
      displayMessage(true, "Error during Claiim, it might not yet be time ");
    }
  };
